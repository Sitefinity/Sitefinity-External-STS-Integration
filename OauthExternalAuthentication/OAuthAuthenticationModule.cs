...
using OauthExternalAuthentication.FacebookProvider;
  ...
  public class OAuthAuthenticationModule : IModule {
    ...
    public void Initialize(ModuleSettings settings) {
      ...
      if (!String.IsNullOrEmpty(oaeConfig.FacebookAPPID) && !String.IsNullOrEmpty(oaeConfig.FacebookAPPSecretKey)) {
        var client = new FacebookOpenAuthenticationProvider(appId: oaeConfig.FacebookAPPID, appSecret: oaeConfig.FacebookAPPSecretKey);

        OpenAuth.AuthenticationClients.Add("facebook", () => client);
      }
