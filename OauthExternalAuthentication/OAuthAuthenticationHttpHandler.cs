    ...
    private void HandleSuccessfullOAuth(AuthenticationResult authResult, System.Web.HttpContext context) {
      ...
      if (currentUser == null) {
        ...
        if (nameParts.Count() == 2)
          SystemManager.RunWithElevatedPrivilege(p => { CreateUser(providerUserId, nameParts[0], nameParts[1], email); });
        else {
          SystemManager.RunWithElevatedPrivilege(p => { CreateUser(providerUserId, string.Empty, string.Empty, email); });
        }
